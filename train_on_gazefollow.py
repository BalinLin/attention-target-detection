...
def train():
    ...
    for ep in range(args.epochs):
        ...
        for batch, (img, dep, face, face_dep, head_channel, gaze_heatmap, gaze_field, eye, gaze, name, gaze_inside, relative_depth) in enumerate(train_loader):
            ...
            gaze_heatmap_pred, attmap, inout_pred, direction, gaze_field_map = model(images, depth, head, faces, face_depth, gaze_field, eye)
            ...
            if (batch != 0 and batch % args.eval_every == 0) or batch+1 == max_steps:
                ...
                with torch.no_grad():
                    ...
                    for val_batch, (val_img, val_dep, val_face, val_face_dep, val_head_channel, val_gaze_heatmap, val_gaze_field, val_eye, cont_gaze, imsize, _) in enumerate(val_loader):
                        ...
                        # predict heatmap(N, 1, 64, 64), mean of attention, in/out
                        val_gaze_heatmap_pred, val_attmap, val_inout_pred, val_direction, val_gaze_field_map = model(val_images, val_depth, val_head, val_faces, val_face_depth, val_gaze_field, val_eye)

...
def testAll():
    f = open(os.path.join("logs", logfolder + ".txt"), 'w')
    for filename in natsort.natsorted(os.listdir(logdir)):
        if filename[-2:] == "pt":
            print("Model:", filename)
            args.model_weights = os.path.join(*["logs", logfolder, filename])
            ...
            f.write("\tAUC:{:.4f}\tmin dist:{:.4f}\tavg dist:{:.4f}\n".format(
                    torch.mean(torch.tensor(AUC)),
                    torch.mean(torch.tensor(min_dist)),
                    torch.mean(torch.tensor(avg_dist))))
    f.close()
